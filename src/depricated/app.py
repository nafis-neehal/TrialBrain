...
def main():
    ...
    user_question = st.text_input(
        "Ask a question about the clinical trial protocol:",
        key="question_input"  # Single consistent key
    )

    # Process input with a button to prevent auto-submit on first Enter
    if st.button("Send", key="send_button"):
        if user_question:
            with st.spinner("Searching for answer..."):
                response = handle_user_input(user_question)

                # Display source documents (unique only)
                if "source_documents" in response:
...
if __name__ == "__main__":
    ...
    # Avoid warnings and potential slowdowns
    os.environ["TOKENIZERS_PARALLELISM"] = "false"
    # Reduce overhead from warnings
